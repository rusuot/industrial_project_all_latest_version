...
function Payment() {
  ...
  const notify1 = () =>
    toast.error(
      "Please fill-up the form correctly && add a product to cart to continue.",
      {
        position: "top-center",
        autoClose: 1800,
  ...
  const handlePincodeBlur = (event) => {
    // UK Postcode Regex: Accounts for most common forms of UK Postcodes
    const postcodeRegex = /^[A-Z]{1,2}[0-9][A-Z0-9]? ?[0-9][A-Z]{2}$/i;

    const inputPincode = event.target.value.trim();

    if (!inputPincode) {
      setPincodeError("Please enter your pincode.");
    } else if (!postcodeRegex.test(inputPincode)) {
      setPincodeError("Please enter a valid UK postcode.");
    } else {
      setPincodeError("");
    }
  };
  ...
  useEffect(() => {
    const storedID = Array.from({ length: 6 }, () => {
  ...
  const TotalAmount = cartContext.items.reduce((total, item) => {
    return total + item.quantity * item.price.current_price;
  }, 0);
  ...
  const onConfirm = () => {
    axios
      .get(
        "https://us-central1-nexus-ecommerce-d1cc9.cloudfunctions.net/sendEmail",
        {
          params: {
            Name: Name,
            Email: Email,
              ...
              <div className="all-data-of-user">
                ...
                <div className="user-data2">
                  ...
                  <div className="user-pincode">
                    <p className="user-pin-number">Post Code*</p>
                    <input
                      type="text"
